...
require_once(ROOT_DIR . 'WebServices/MicrosoftLogin/microsoftConfig.php');
class ExternalAuthLoginPresenter
{
    ...
    public function PageLoad()
    {
		...
		if ($this->page->GetType() == 'office') {
            $this->ProcessOfficeSingleSignOn();
        }
    }
    ...
    private function ProcessSocialSingleSignOn($page)
    {
	 ...
	 private function ProcessOfficeSingleSignOn()
    {
        $code = filter_input(INPUT_GET,'code');
        Log::Debug('Logging in with office. Code=%s', $code);
        $result = file_get_contents(ROOT."WebServices/MicrosoftLogin/microsoftLogin.php?code=".$code);
		
        $profile = json_decode($result);
        $requiredDomainValidator = new RequiredEmailDomainValidator($profile->userPrincipalName);
        $requiredDomainValidator->Validate();
        if (!$requiredDomainValidator->IsValid()) {
            Log::Debug('Social login with invalid domain. %s', $profile->userPrincipalName);
            $this->page->ShowError(array(Resources::GetInstance()->GetString('InvalidEmailDomain')));
            return;
        }

        Log::Debug('Social login successful. Email=%s', $profile->userPrincipalName);
        $this->registration->Synchronize(new AuthenticatedUser($profile->userPrincipalName,
        ...
        $this->authentication->Login($profile->userPrincipalName, new WebLoginContext(new LoginData()));
        LoginRedirector::Redirect($this->page);
    }
}
